function updateInput() {
  MOUSE.cell = cellAtI(ROWOFF[MOUSE.gy] + MOUSE.gx);
...
function updateTime() {
  now = performance.now();
  nowSec = now / 1000;
  dt = Math.min((nowSec - last) / 0.03, 3);
  last = nowSec;
  ticks++;
...
function updateParticules() {
  ...
  liqCells = 0;
  if (ISRAINING) {
    for (let i = 0; i < RAINPOW; i++) launchCells(cellKeys[TYPEINDEX], r_range(1, CANVW - 1), 1, 1, 1, false, [0, 0]);
  }
  for (let i = 0; i < cellEmitters.length; i++) cellEmitters[i].update();
...
function updateParticuleSounds() {
  ...
  else playParticleSound(au.fallingSand, impactCells > 0 ? clamp(impactCells / 50, 0.1, 1) : 0);
  impactCells = 0;
}
