p.updateMovement = function () {
  ...
  for (let i = 0; i < steps; i++) {
    curX += xStep;
    curY += yStep;
    const realX = Math.round(curX),
      realY = Math.round(curY);
    ...
    if (this.isProjectile && hit && this.user !== hit.ent && hit.physT === "SOLID" && hit.type !== 'JET') {
      launchCells("SMOKE", this.x * PIXELSIZE, this.y * PIXELSIZE, 10, 10);
      if (hit.ent) {
        hit.ent.hit(r_range(6, 13), this);
        if (shouldBurn(this, hit)) hit.setToFire();
        this.toRemove();
      } else this.endLifetime();
      return;
    }
    ...
    if (this.douse && hit.type !== "FISH" && hit.type != "BEE") {
      if (hit.brnpwr) this.setType("STEAM");
      else hit.setWet(100, this.type === "BUBBLE" ? "WATER" : this.type);
    }
    if (shouldBurn(this, hit)) {
      hit.setToFire(this.type);
    }
    if (shouldBurn(hit, this)) {
      this.setToFire(this.type);
