p.updateLiquidVelocity = function (g) {
  if (this.type === "WATER") {
    if (!g && dice(20000)) {
      ...
      let newClr = addColor(curClr, this.baseColor, 0.5);
      n.setColor(newClr);
      return;
    }
    if (dice(400) && !cellAtI(ROWOFF[this.y - 1] + this.x, this)) {
      ...
      const n = this.replace("BUBBLE", type);
      let curClr = n.baseColor;
      let newClr = addColor(curClr, prvClr, 0.5);
      n.setColor(newClr);
      return;
    }
  }
  if (this.updT != "STATIC") this.velY += GRAVITY;
};
...
p.updateSolidVelocity = function (g) {
  ...
  if (g && g.physT === "LIQUID") {
    this.velY += GRAVITY;
    this.velY *= 1 - g.dns * 0.1;
    this.velX *= 0.99;
    return;
  }
  if (!this.hasTouchedBorder) {
    this.hasTouchedBorder = this.y >= GH - 1 || (g && (g.updT === "STATIC" || g.hasTouchedBorder));
    if (this.hasTouchedBorder && !this.inWater && this.updT !== "ALIVE") {
      this.velX = 0;
      impactCells++;
    }
  } else if (this.hasTouchedBorder > 0 && !g && this.y < GH - 1) this.hasTouchedBorder--;
  ...
  if (grounded && this.updT !== "ALIVE") {
    ...
    this.velY += this.ownGravity ?? GRAVITY;
    if (this.id % 500 == 0) impactCells++;
  }
  if (this.type === "GBLADES") this.velX = 0;
};

p.updateVelocity = function () {
  const pt = this.physT;
  if (this.type === "FLY") return;
