const UPDATE_HANDLERS = {
  ...
  FLY: (p) => p.updateFly(p.newX, p.newY),
  GBLADE: (p) => p.updateGrassBlade(p.newX, p.newY),
  ENT: (p) => p.updateEntCell(p.newX, p.newY),
};
...
p.trySetToSleep = function () {
  if (this.burning || this.frozen || this.cor || this.wet || this.warm) return false;
  if (this.updT === "STATIC") return true;
...
p.updateType = function () {
  ...
  if (this.physT !== "LIQUID" || FRAME % 100 === 0 || (fps < 30 && this.timeAlive < 5)) {
    if (this.isBasic && this.lt === Infinity && !this.cor && !this.freeze && !this.burning && this.trySetToSleep()) {
      this.isAsleep = true;
      return;
    }
  }
  this.getNeighborCells();
  if (this.physT === "SOLID" || this.physT === "STATIC") {
    if (FRAME % 50 === 0) {
      var curInWater = false;
      this.inWater = false;
      for (let i = 0; i < this.neighbors.length; i++) {
        if (this.neighbors[i].physT === "LIQUID" && !this.neighbors[i].frozen) {
          curInWater = true;
          break;
        }
      }
      if (!curInWater) this.timeInWater = 0;
      else this.inWater = true;
    }
    if (this.inWater) this.timeInWater++;
  }
  if (this.type === "DUST" && FRAME % 2 == 0) return;
  if (this.type === "PROJ") {
    const c = new Cell(this.x * PIXELSIZE, (this.y + 1) * PIXELSIZE, "JET");
  }
  ...
  if (this.physT === "LIQUID") {
    ...
    if (this.type === "LAVA") this.LavaEffect(this.x, this.y);
    return;
...
p.setType = function (newType, transformType = null) {
  if (this.ent && newType !== this.type) return;
  ...
  this.lt = this.properties.lt * f_range(0.5, 1.5);
...
p.setTypeSpecifics = function () {
  if (ISGAME && !this.properties.kn)
    setTimeout(() => {
      discoverType(this);
    }, 50);
  if (this.updT == "STATIC" || this.type === "FLY") {
    this.velY = 0;
    this.velX = 0;
  }
  if (this.type == "FISH") {
    ...
    this.alpha = f_range(0.1, 0.2);
    if (dice(10)) this.setColor(addColor(this.properties.color, "rgb(132, 132, 132)", r_range(1, 10)));
    else this.setColor(this.properties.color);
  } else if (this.type === "SMOKE") {
    this.size = r_range(2, 10);
    this.alpha = f_range(0.05, 0.1);
