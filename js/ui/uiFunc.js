function switchUiPage(newPageIndex) {
  uiPageIndex = newPageIndex;
}
function setNewType(newIndex) {
  if (typeof newIndex === "string") {
    newIndex = cellKeys.indexOf(newIndex);
    if (newIndex === -1) {
      console.log("wrong string index in setNewType");
      return 0;
    }
  }
  ENTINDEX = -1;
  ...
  if (ISGAME) {
    ...
    if (!CELL_PROPERTIES[cellKeys[newIndex]].kn) return 1;
  }
  if (ISGAME && !CELL_PROPERTIES[cellKeys[newIndex]].kn) return 1;
  switchBrushAction(null);
  uiLayerIndex = 1;
  TYPEINDEX = newIndex;
  for (const b of uiPagesButtons[uiPageIndex].buttons) {
    if (b.label == cellKeys[newIndex]) {
      typeButton = b;
      b.new = false;
    }
  }
...
function setNewBrushSize(newPercentile) {
  BRUSHSIZE = Math.floor((Math.min(GW, GH) / 3 / 100) * newPercentile);
  settingBrushSize = true;
}
function setNewBrushType(newType) {
  BRUSHTYPE = BRUSHTYPE == "RECT" ? "DISC" : "RECT";
}
function setNewGravity(newGravity) {
  if (Math.sign(newGravity != GRAVITY)) {
    for (const c of activeCells) c.hasTouchedBorder = false;
  }
  GRAVITY = newGravity;
}
...
function switchGridMode(newGridMode) {
  gridMode = newGridMode;
}
function setRAINPOW(newIntensity) {
  RAINPOW = newIntensity / PIXELSIZE;
}
function setObsurity(newIntensity) {
  OBSCURITY = newIntensity;
}
...
function fillScreen() {
  ...
  for (let x = 0; x < GW; x++) {
    for (let y = yStart; y < GH; y++) {
      const c = new Cell(x, y, cellKeys[TYPEINDEX]);
      c.hasTouchedBorder = true;
    }
  }
}
