function launchCell(type, x, y, clr, vx, vy, isRandomized, lt = null) {
  var newP = new Cell(x, y, type);
  if (clr) {
    if (type === "ANT") newP.setColor(randomizeColor(clr, 10));
    else if (randomizeChance > 0 && isRandomized && dice(randomizeChance)) newP.setColor(randomizeColor(clr, 5));
...
function launchCells(type = "SAND", px = MOUSE.x, py = MOUSE.y, rx = BRUSHSIZE, ry = BRUSHSIZE, isDisc = BRUSHTYPE == BRUSHTYPES.DISC, vel = [MOUSE.dx, MOUSE.dy], lt = null) {
  ...
  if (ENTINDEX != -1 && MOUSE.pressed) {
    if (ENTINDEX > 0) {
      const data = getEntOfIndex(ENTINDEX);
      SELENT = new data.ctor(Math.round(px / PIXELSIZE), Math.round(py / PIXELSIZE), data, "MOB");
      entities.push(SELENT);
    } else {
      if (!PLAYER) PLAYER = new Player(Math.round(px / PIXELSIZE), Math.round(py / PIXELSIZE));
      SELENT = PLAYER;
    }
    return;
  }
  ...
  const addedX = vel[0] ? (vel[0] / PIXELSIZE) * (CELL_PROPERTIES[type].physT == PHYSTYPES.LIQUID ? 0.4 : 0.05) : 0;
  const addedY = vel[1] ? (vel[1] / PIXELSIZE) * (CELL_PROPERTIES[type].physT == PHYSTYPES.LIQUID ? 0.4 : 0.05) : 0;
  ...
  if (type === "RAINBOW") color = getRainbowColor(FRAME, 0.1);
  else if (isRandomized) color = addColor(CELL_PROPERTIES[type].color, getRainbowColor(FRAME, 0.1), 0.1);
  ...
  if (rx === 1 && ry === 1) {
    ...
    const c = launchCell(type, ix, iy, color, addedX, addedY, isRandomized, lt);
    if (c && c.active) launchedCells.push(c);
...
function launchCellsRect(type, xp, yp, w, h) {
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (type === "RAINBOW") newP.setColor(getRainbowColor(FRAME, 0.1));
    }
  }
}
