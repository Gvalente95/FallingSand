...
let actionBtnH = actionBtnW * 0.4;
let btnW = actionBtnW;
let btnH = isMobile ? btnW * 0.6 : 30;
...
let CELL_PROPERTIES = {
  ...
  ["ICE"]: { color: "rgba(66, 180, 205, 1)", lt: Infinity, brn: 400, brnpwr: 0, douse: 0, freeze: 1, cor: 0, physT: "SOLID", updT: "DYNAMIC", dns: 10, spread: 2, expl: 0, kn: 0, rclr: 0, fin: 0.4, fout: 0.2 },
  ...
  ["JET"]: { color: "rgba(199, 198, 189, 1)", lt: 0.3, brn: 10, brnpwr: 0, douse: 0, freeze: 0, cor: 0, physT: "SOLID", updT: "DYNAMIC", dns: 50, spread: 0, expl: 0, kn: 0, rclr: 5, fin: 0.2, fout: 0.5 },
  ...
  ["PROJ"]: { color: "rgba(179, 213, 29, 1)", lt: 5, brn: 999, brnpwr: 800, douse: 0, freeze: 0, cor: 0, physT: "SOLID", updT: "DYNAMIC", dns: 50, spread: 0, expl: 2, kn: 0, rclr: 5, fin: 0, fout: 0 },

  ["WATER"]: { color: "rgb(0,91, 255)", lt: Infinity, brn: 0, brnpwr: 0, douse: 1, freeze: 0, cor: 0, physT: "LIQUID", updT: "DYNAMIC", dns: 2, spread: 40, expl: 0, kn: 1, rclr: 0, fin: 0, fout: 0 },
  ...
  ["CHEMX"]: { color: "rgba(41, 159, 58, 1)", lt: Infinity, brn: 0, brnpwr: 0, douse: 1, freeze: 0, cor: 1500, physT: "LIQUID", updT: "DYNAMIC", dns: 1.9, spread: 6, expl: 0, kn: 0, rclr: 0, fin: 0, fout: 0 },
  ...
  ["FIRE"]: { color: "rgb(214, 113, 40)", lt: 0.3, brn: 0, brnpwr: 1000, douse: 0, freeze: 0, cor: 0, physT: "GAS", updT: "DYNAMIC", dns: 1, spread: 0, expl: 0, kn: 1, rclr: 10, fin: 0.05, fout: 0.4 },
  ["HESTIA"]: { color: "rgba(199, 56, 199, 1)", lt: 0.3, brn: 0, brnpwr: 1000, douse: 0, freeze: 0, cor: 0, physT: "GAS", updT: "DYNAMIC", dns: 1, spread: 0, expl: 0, kn: 1, rclr: 10, fin: 0.05, fout: 0.4 },
  ...
  ["BOLT"]: { color: "rgb(212, 255, 0)", lt: 0.1, brn: 0, brnpwr: 0, douse: 0, freeze: 0, cor: 0, physT: "GAS", updT: "DYNAMIC", dns: 100, spread: 0, expl: 0, kn: 0, rclr: 10, fin: 0.2, fout: 0.1 },
  ...
  ["PYROGEL"]: { color: "rgb(39, 218, 200)", lt: 0.4, brn: 0, brnpwr: 0, douse: 0, freeze: 1, cor: 0, physT: "GAS", updT: "DYNAMIC", dns: 10, spread: 2, expl: 0, kn: 0, rclr: 0, fin: 0.4, fout: 0.5 },
  ["STEAM"]: { color: "rgba(210, 218, 227, 1)", lt: 4, brn: 0, brnpwr: 0, douse: 0, freeze: 0, cor: 0, physT: "GAS", updT: "DYNAMIC", dns: 1, spread: 0, expl: 0, kn: 0, rclr: 0, fin: 0, fout: 0.2 },
  ...
  ["FLY"]: { color: "rgba(40, 125, 75, 1)", lt: 20, brn: 0, brnpwr: 0, douse: 0, freeze: 0, cor: 0, physT: "SOLID", updT: "ALIVE", dns: 50, spread: 0, expl: 0, kn: 0, rclr: 10, fin: 0.2, fout: 0.5 },
  ...
  ["TORCH"]: { color: "rgb(214, 113, 40)", lt: Infinity, brn: 0, brnpwr: 1000, douse: 0, freeze: 0, cor: 0, physT: "SOLID", updT: "STATIC", dns: 30, spread: 0, expl: 0, kn: 0, rclr: 10, fin: 0.5, fout: 0.5 },
  ["WOOD"]: { color: "rgb(61, 41, 37)", lt: Infinity, brn: 700, brnpwr: 0, douse: 0, freeze: 0, cor: 0, physT: "SOLID", updT: "STATIC", dns: 40, spread: 0, expl: 0, kn: 0, rclr: 0, fin: 0, fout: 0 },
  ...
  ["ENTITY"]: { color: "rgba(177, 136, 136, 1)", lt: Infinity, brn: 900, brnpwr: 0, douse: 0, freeze: 0, cor: 0, physT: "SOLID", updT: "STATIC", dns: 1, spread: 0, expl: 0, kn: 0, rclr: 0, fin: 0, fout: 0 },
};
...
function tagMaskOf(tags) {
  return tags.reduce((mask, tag) => {
    const index = TAG_INDEX[tag];
    return index !== undefined ? mask | (1 << index) : mask;
  }, 0);
}
function addTag(type, color = setBrightness[CELL_PROPERTIES[cellKeys[type]].color]) {
  const brightColor = setBrightness(color, 150);
  TAGS.push({ type, color: brightColor });
  TAG_INDEX[type] = TAGS.length - 1;
}
function hasTag(type, tag) {
  const p = CELL_PROPERTIES[type];
  return !!(p.tagMask & (1 << TAG_INDEX[tag]));
}
function anyTag(type, tags) {
  const m = tagMaskOf(tags);
  return (CELL_PROPERTIES[type].tagMask & m) !== 0;
}
function allTags(type, tags) {
  const m = tagMaskOf(tags);
  return (CELL_PROPERTIES[type].tagMask & m) === m;
}
...
function typesWithAny(tags) {
  const m = tagMaskOf(tags);
  return Object.keys(CELL_PROPERTIES).filter((k) => (CELL_PROPERTIES[k].tagMask & m) !== 0);
}
function typesWithAll(tags) {
  const m = tagMaskOf(tags);
  return Object.keys(CELL_PROPERTIES).filter((k) => (CELL_PROPERTIES[k].tagMask & m) === m);
}
