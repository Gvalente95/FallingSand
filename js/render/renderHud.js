...
function updateHUD(){
  ...
  const topNow = `x${MOUSEX},y${MOUSEY} gx${MOUSEGRIDX},gy${MOUSEGRIDY}  Pxls: ${activeParticles.length} Tm: ${time}`;
  ...
  if (PXATMOUSE && !isMobile) {
    ...
    let dpx = `x${x} y${y} vx${vx} vy${vy}`;
    const tm = (PXATMOUSE.timeAlive);
    const tmStr = Math.round(tm*10)/10;
    const parts = [
      `${dpx} Type: ${PXATMOUSE.type} TM: ${tmStr}`
    ];
